---
title: Organization spend limits, snapshot storage consumption API, One-click Neon MCP setup for Kiro, durable AI agents, docs for AI agents
---
...
Building on the recent [snapshot size fields added to snapshot API responses](/docs/changelog/2026-04-17#snapshot-api-responses-include-storage-size-fields), the [Retrieve project consumption metrics](https://api-docs.neon.tech/reference/getconsumptionhistoryperprojectv2) endpoint now supports a **`snapshot_storage_bytes_month`** in the `metrics` parameter. Use it to track monthly snapshot storage in your project-level consumption reporting.
...
We've been optimizing our docs for AI agents. The post [**Agents grew up, so did our docs**](https://neon.com/blog/agents-grew-up-so-did-our-docs) covers what worked, what didn't, and what we're still figuring out. It also walks you through what we put in place: MDX converted to Markdown at build time, middleware that serves Markdown for agent-shaped requests, the same doc paths with a `.md` suffix, a structured [`llms.txt`](https://neon.com/docs/llms.txt) index.

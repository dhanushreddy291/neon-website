...
The guide [Simulate production load using Neon branching and k6](/guides/k6-load-test-neon-branching) walks through the full flow. It uses a branch named **`load-test-branch`** in the examples; with the [Neon CLI](/docs/reference/cli-install), a minimal create from your real data branch (here **`main`**; use whatever you run in production) looks like this:
...
Use the new branch’s connection URI in **`DATABASE_URL`** (or **`neon connection-string`**), then run your app and k6 against that database. For more branching workflows, see [Branching with the Neon CLI](/docs/guides/branching-neon-cli).
